document.addEventListener("DOMContentLoaded", function () {
    ...
    if (phoneInput) {
        phoneInput.addEventListener("input", function() {
            ...
            if (!phoneRegex.test(this.value.trim()) && this.value.trim().length > 0) {
                this.classList.add("error");
                errorElement.textContent = "Phone must start with 07 and be 10 digits.";
            } else {
                this.classList.remove("error");
                errorElement.textContent = "";
            }
    ...
    form.addEventListener("submit", function (event) {
        ...
        if (!phoneRegex.test(phone)) {
            isValid = false;
            document.getElementById("phone-error").textContent = "Phone number must start with 07 and be 10 digits.";
